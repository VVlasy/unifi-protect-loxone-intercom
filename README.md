...
video — with a single container and four lines of config.
...
There's also an **optional, experimental** fix to force a portrait-locked UniFi
doorbell to output **landscape** — see section 9.
...
## 9. Optional: force the doorbell to landscape (experimental)

UniFi doorbells are **portrait by design** and hardcode a 90° "hallway" rotation in
firmware. Protect **cannot disable it** for doorbells. If yours watches a wide scene
(street, driveway) you may want it **landscape** like your other cameras.

A watchdog in this container can correct the orientation **at the source** (so
Protect, recordings *and* the Loxone feed are all fixed), by SSHing to the camera and
applying an `LD_PRELOAD` shim + ISP flip. It pokes **undocumented firmware internals**
of a specific model and is **fully opt-in** (`ROTATION_FIX_ENABLED=0` by default).
The full how/why, prerequisites and caveats are in
**[ADVANCED.md → Camera landscape fix](ADVANCED.md#camera-landscape-fix)**.

> Simpler alternative: if you only need *Loxone's* view rotated, set
> `MJPEG_TRANSPOSE` and leave the camera stock.
...
- **The landscape fix (section 9)** is the most experimental piece — undocumented,
  model-specific, off by default, and does not affect the SIP/video bridge.
